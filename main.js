...
function sbtgnt(){
	      ...
	      if(strUser!=0)
	        {
	           ...
	           e.setAttribute(name,strUser);
	            e.setAttribute("style","background-color: none;");
	            jsonMap['answerlist['+i+'].emailId']=email;
	            jsonMap['answerlist['+i+'].questionNumber']=name;
	            jsonMap['answerlist['+i+'].answerValue'] =strUser;
	            jsonMap['answerlist['+i+'].id'] =0;
	        }
        else{
    ...
    var comments = $("#excite").val();
    var jsonObj = Object.assign({}, jsonMap);
    jsonObj['comments'] =comments;
    $.ajax({ 
    	url: '/OnlineTest/saveUserResponse',
        type: 'POST',
        data: jsonObj, 
        async: false,    
        cache: false,      
...
{   
    ...
    /*$.ajax({
		url : "/OnlineTest/displayMainContent",
		data : {emailId : email},
		//dataType : 'html',
		async:false,
		cache:false,
